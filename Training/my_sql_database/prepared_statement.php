<html>
    <body>
        ...
        <h1>This is executed by prepared statement</h1>
        ...
        <?php
            ...
            {   // exploding string field to array 
                ...
                $stmt = $conn->prepare("INSERT INTO employe_info (firstname, lastname, email) VALUES (?, ?, ?)");  // prepared statement query is generated only once then
                 
                 // inserting each array value to data using loop
                for($i=0; $i<$total_value ; $i++)
                {  
                    $stmt->bind_param("sss", $arr_fname[$i], $arr_lname[$i], $arr_email[$i]);
                    $stmt->execute();
                }
                $stmt->close();
